...
Floor: PHP `^8.3`, Laravel `^13.0`. Recommend PHP 8.4 features
(property hooks, asymmetric visibility) opportunistically where they
materially shrink code, but never gate behind 8.4-only features without
a parallel 8.3 path. `#[\Override]` attribute mandatory on every
overriding method.
