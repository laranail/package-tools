...
/**
 * HasAssetGroups - Asset group publishing
 *
 * Enables publishing assets in organized groups
 */
trait HasAssetGroups
{
    ...
    public function hasAssetGroup(string $groupName, array $config = []): static
    {
        ...
        // Registration is complete — line above stored the group config in
        // $this->assetGroups, which PackageServiceProvider's ProcessAssets
        // iterates at boot time. No separate publish call is needed here.
        // (Previously called an abstract publishAssets() that collided with
        // HasAssetPublisher::publishAssets() under the ConfiguresAssets
        // aggregator — see ADR-004.)
    ...
    // (Previously declared `abstract publishAssets()` here. Removed in Phase 3
    // because (a) it collided with HasAssetPublisher::publishAssets() once
    // both traits coexisted under ConfiguresAssets, and (b) the registration
    // it required is already handled by storing into $this->assetGroups.)
}
