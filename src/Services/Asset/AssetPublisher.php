...
/**
 * AssetPublisher - Asset publishing orchestration
 *
 * Handles publishing of assets from package to application
 * Uses configuration-driven patterns for publish tags
 */
class AssetPublisher implements PublisherInterface
{
    protected array $published = [];

    public function __construct(protected ServiceProvider $provider, protected AssetRegistry $registry, protected ConfigService $config, protected PatternResolver $patternResolver) {}
    ...
    public function publish(string $source, string $target, string $tag): void
    {
        ...
        $this->provider->publishes([$source => $target], $tag);
    ...
    public function publishGroup(array $sources, string $tag): void
    {
        ...
        if ($validSources !== []) {
            $this->provider->publishes($validSources, $tag);
